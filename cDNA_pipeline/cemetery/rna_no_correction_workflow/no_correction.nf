...
params.reads_fq = '/mnt/gpfs3_amd/condo/mteb223/bag222/data/cdna_comparison_project/2022_test_direct_rna_data/*.fastq'
...
workflow {
    ...
    MAPPING_QC_DIRECT_RNA(reads_fq, MAKE_INDEX.out)

    // PYCOQC(MAPPING_QC_DIRECT_RNA.out.id, reads_txt, MAPPING_QC_DIRECT_RNA.out.bam_all, MAPPING_QC_DIRECT_RNA.out.bai_all)

    // RSEQC(MAPPING.out.id, MAPPING.out.bam, MAPPING.out.bai, housekeep, bed)

    DOWNSAMPLING(MAPPING_QC_DIRECT_RNA.out.id, MAPPING_QC_DIRECT_RNA.out.bam_mapped, downsizes)

...
# PDF device
pdf("../../../figures/bernardo/04.deseq2/AD_transcript_level_plots_AD_vs_CT_no_intron.pdf")
...
for (transcript_id in converter$transcript_id) {
    ...
    if (!(gene_id %in% list_gene_ids)) {
        ...
        plot = ggplot(d, aes(x=condition, y=count, fill=condition)) + ylab("Normalized DESeq2 counts") + 
